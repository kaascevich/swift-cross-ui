...
/// ``AppBackend/createPopoverMenu()``,
/// ``AppBackend/updatePopoverMenu(_:content:environment:)``, and
/// ``AppBackend/updateButton(_:label:environment:action:)``.
public enum MenuImplementationStyle {
    ...
    /// ``AppBackend/showPopoverMenu(_:at:relativeTo:closeHandler:)``. For
    /// these backends, ``AppBackend/createPopoverMenu()`` is not called
